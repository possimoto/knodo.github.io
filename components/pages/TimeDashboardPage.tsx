...
const TimerInput: React.FC<{ activities: Activity[]; onAdd: (a: Omit<Activity,'id'>) => void }> = ({ activities, onAdd }) => {
  ...
  const handleStop = () => {
    ...
    if (endMin > startMin) onAdd({ category: selectedCategory, title: title || nameOf(selectedCategory), start: startMin, end: endMin });
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={isRunning ? handleStop : handleStart}
            className={`px-6 py-2 rounded-full font-semibold text-white transition-colors ${isRunning ? 'bg-red-500 hover:bg-red-600' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
            {isRunning ? '■ 정지' : '▶ 시작'}
          </button>
  ...
  return (
    <div className="relative">
      ...
      <div className="relative h-10 bg-gray-700 rounded-lg overflow-hidden">
        {activities.map(a => {
          const left = Math.max(0, ((a.start-DAY_START)/DAY_SPAN)*100);
...
const CategoryPie: React.FC<{ activities: Activity[] }> = ({ activities }) => {
  ...
  for (const a of activities) { if (a.category === 'rest') continue; totals[a.category] = (totals[a.category]??0)+(a.end-a.start); }
...
const ActivityList: React.FC<{ activities: Activity[]; onDelete: (id: number) => void }> = ({ activities, onDelete }) => {
...
const TimeDashboardPage: React.FC = () => {
  ...
  const handleAdd = (a: Omit<Activity,'id'>) => { setActivities(prev => [...prev, { ...a, id: nextId }]); setNextId(n => n+1); };
  const handleDelete = (id: number) => setActivities(prev => prev.filter(a => a.id !== id));
  const totalActiveMin = activities.filter(a => a.category !== 'rest').reduce((s,a) => s+a.end-a.start, 0);
  const topCategory = (() => {
    const best = CATEGORIES.filter(c => c.id !== 'rest').reduce((b,c) => {
      const t = activities.filter(a => a.category===c.id).reduce((s,a) => s+a.end-a.start,0);
      const bt = activities.filter(a => a.category===b.id).reduce((s,a) => s+a.end-a.start,0);
      return t > bt ? c : b;
    }, CATEGORIES[0]);
